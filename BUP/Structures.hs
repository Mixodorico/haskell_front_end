...
extrVar id (x@(Var a _ _ _):xs) | id==a =   x
...
insVar x@(Var a _ _ _) xs   | (searchVar a xs) =   x:(deleteVar a xs)
      | otherwise = x:xs
...
resetEnvV [] = []
resetEnvV ((Var a t _ p):xs) =  (Var a t False p) : resetEnvV xs
